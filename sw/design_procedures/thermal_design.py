...
def get_switch_thermals(
    t_a, t_j, p_sw_bud, r_jb, r_jc, r_sa, area_hs, num_vias, output_path
):
    """_summary_
    ...
    points = [
        [i, j, i + j, get_r_ja(i, j, r_jb, r_jc, r_sa, area_hs, num_vias)]
        for j in np.linspace(1e-5, 5e-2, 100)
        for i in np.linspace(1e-5, 5e-2, 100)
    ]
    df = pd.DataFrame(
        points,
        columns=["AREA_FCU (m^2)", "AREA_BCU (m^2)", "AREA_TOT (m^2)", "R_JA (C/W)"],
    )
    # Constrain by maximum resistance
    df = df[df["R_JA (C/W)"] <= target_r_ja]

    fig = plt.figure()
    ax = fig.add_subplot(projection="3d")
    ax.scatter(
        np.multiply(df["AREA_FCU (m^2)"], 1000000),
        np.multiply(df["AREA_BCU (m^2)"], 1000000),
        df["R_JA (C/W)"],
        c=df["R_JA (C/W)"],
    )
    ax.set_title("$R_{JA}$ as a function of TCU area and BCU area")
    ax.view_init(15, 45)
    ax.set_xlabel("FCU ($mm^{2}$)")
    ax.set_ylabel("BCU ($mm^{2}$)")
    ax.set_zlabel("$R_{JA}$ (*C/W)")
    ax.set_zlim(np.min(df["R_JA (C/W)"]), target_r_ja)
    plt.tight_layout()
    plt.show()
    fig.savefig(output_path + "/06_sw_thermal_map.png")
    plt.close()

    # Get the min area needed to hit the R_JA
    df = df[(df["R_JA (C/W)"] == df["R_JA (C/W)"].max())]
    return np.min(df["AREA_TOT (m^2)"])
