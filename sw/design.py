...
if __name__ == "__main__":
    ...
    print(f"STEP 2")
    tau = c_oss * r_ds_on
    print(f"FOM for this switch: {tau * 10**9} (ns).")
    print(
    ...
    v_in_combos = np.linspace(v_in_range[0], v_in_range[1], num=100, endpoint=False)
    v_out_combos = np.linspace(v_out_range[0], v_out_range[1], num=100, endpoint=False)
    ...
    ax.set_zlabel("F_S_MAX (Hz)")

    plt.show()

    # Determine minimum frequency allowed across the map.
    print(
        f"Minimum frequency for the converter: {math.floor(min(z_f_s)) * 10**-3} kHz "
    ...
    # straightforward to determine when ci, co, l are minimized.
    ci_min = []
    co_min = []
    l_min = []
    ...
    print(f"Minimum C_IN: {np.min(ci_min) * 10**6} uF")
    print(f"Minimum C_OUT: {np.min(co_min) * 10**6} uF")
    print(f"Minimum L: {np.min(l_min) * 10**6} uH")
    print(
