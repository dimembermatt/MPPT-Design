 ...
 * @brief Feedback Control MPPT algorithm driver.
 * @version 0.2.0
 * @date 2023-12-05
 * 
...
class FC final : public MPPT {
    public:
        ...
        void step_algorithm(void) {
            ...
            float slope = delta_array_power / delta_array_voltage;

            #define STRIDE 0.1 // TODO: support variable stride
            #define ERROR 0.05 // TODO: support variable error
            if (delta_array_voltage == 0) {
                reference_voltage += STRIDE; // Prevent divide by 0 exception.
            } else if (abs(slope) > ERROR) {
                if (slope > 0) {
